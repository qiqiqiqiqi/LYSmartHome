package com.jb.jb_library.picker;
...
public class TimeSelector {
    ...
    private String FORMAT_STR = "yyyy-MM-dd HH:mm"; //时间样式
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            ...
            int width = ScreenSizeUtil.getInstance(context).getScreenWidth();
            lp.width = width;
    ...
    public void setStartTime() {
        selectedCalender.set(Calendar.YEAR, DateUtil.getCurrentYear());
        initMonthChange();
    }
    ...
    private void loadComponent() {
        ...
        mCurrYear = c.get(Calendar.YEAR);
        mCurrMonth = c.get(Calendar.MONTH) + 1;
        mCurrDay = c.get(Calendar.DAY_OF_MONTH);
        mCurrHour = c.get(Calendar.HOUR_OF_DAY);
        mCurrMinute = c.get(Calendar.MINUTE);

        year_pv.setSelected(String.valueOf(mCurrYear));
        setStartTime();
        excuteScroll();
    }
    ...
    private void initMonthChange() {
        ...
        selectedCalender.set(mCurrYear, mCurrMonth - 1, mCurrDay);
        month_pv.setData(month);
        month_pv.setSelected(mCurrMonth - 1);
    ...
    private void initDayChange() {
        ...
        selectedCalender.set(Calendar.DAY_OF_MONTH, mCurrDay);
        day_pv.setData(day);
        day_pv.setSelected(mCurrDay - 1);
    ...
    private void initHourDayChange() {
        if ((scrollUnits & SCROLLTYPE.HOUR.value) == SCROLLTYPE.HOUR.value) {
            hour.clear();
            int selectedYear = selectedCalender.get(Calendar.YEAR);
            int selectedMonth = selectedCalender.get(Calendar.MONTH) + 1;
            ...
            selectedCalender.set(Calendar.HOUR_OF_DAY, mCurrHour);
            hour_pv.setData(hour);
            hour_pv.setSelected(mCurrHour);
    ...
    private void initMinuteChange() {
        if ((scrollUnits & SCROLLTYPE.MINUTE.value) == SCROLLTYPE.MINUTE.value) {
            ...
            selectedCalender.set(Calendar.MINUTE, mCurrMinute);
            minute_pv.setData(minute);
            minute_pv.setSelected(mCurrMinute);
        }
    }
