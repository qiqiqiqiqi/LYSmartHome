...
import com.jb.jb_library.util.DecimalUtil;
 ...
 */
public class CommonClipImageView extends ImageView implements View.OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener{
	...
	public CommonClipImageView(Context context) {
		this(context, null);
	}

	public CommonClipImageView(Context context, AttributeSet attr) {
		this(context, attr, 0);
	}

	public CommonClipImageView(Context context, AttributeSet attr, int defStyle) {
	...
	private class AnimatedZoomRunnable implements Runnable {
		...
		public void run() {
			...
			if (((deltaScale > 1f) && (currentScale < targetZoom))
				...
				// again
				postOnAnimation(CommonClipImageView.this, this);
	...
	public Bitmap clip() {
		...
		float ratio = 1.0f;
		if (getWidth() > 720) {
			ratio = DecimalUtil.devide(720, getWidth(), 2);
		}
		// EGLoger.i("aa", "ratio " + ratio);
		// EGLoger.i("aa", "borderlength " + borderlength + " getWidth() " +
		// getWidth());
		Matrix matrix = new Matrix();
		matrix.postScale(ratio, ratio); // 长和宽放大缩小的比例

		int mgH = borderlength;
		// bitmap = Bitmap.createBitmap(bitmap, (getWidth() - borderlength) / 2,
		// (getHeight() - borderlength) / 2, borderlength, borderlength);
		bitmap = Bitmap.createBitmap(bitmap, (getWidth() - borderlength) / 2,
				(getHeight() - mgH) / 2, borderlength, mgH, matrix, true);
		// bitmap =
		// Bitmap.createBitmap(bitmap,0,0,bitmap.getWidth(),bitmap.getHeight(),matrix,true);
		// EGLoger.i("aa", "width " + bitmap.getWidth() + " height " +
		// bitmap.getHeight());
		return bitmap;
	}
}
